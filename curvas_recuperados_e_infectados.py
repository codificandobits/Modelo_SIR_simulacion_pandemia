...
def plot_filled_frame(i,count,t,ii,rr):
    ...
    plt.axis([0, 175.5, 0, 1.1])
    ax.plot(t[0:i+count],rr[0:i+count], color=(green), alpha=1, lw=0.5, label='recuperados')
    ax.fill_between(t[0:i+count],rr[0:i+count], y2=0, color=(green), alpha=0.3)
    ax.plot(t[0:i+count],ii[0:i+count], color=(red), alpha=1, lw=0.5, label='infectados')
    ax.fill_between(t[0:i+count],ii[0:i+count], y2=0, color=(red), alpha=0.6)
    ax.set_facecolor((0,0,0))

    # Legenda
    legend = ax.legend(loc=(0.7,0.82),facecolor='k',edgecolor='k')
    ...
    plt.yticks(np.arange(0, 1.1, step=0.2),['', '20', '40', '60', '80', '100'])
    ...
    plt.xticks(np.arange(0, 175+1, step=20))
...
# Create movie
# ffmpeg -r 30 -f image2 -s 1920x1080 -i img_%03d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p 26_500.mp4
